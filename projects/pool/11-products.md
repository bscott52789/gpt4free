...
|---|---|---|
| Salt | Pool salt, 99%+ sodium chloride, 40 lb bags (Morton Pool Salt, Diamond Crystal Pool Salt, Clorox Pool Salt) | Rock salt, softener salt with additives, salt with anti‑caking agents |
| pH / TA down | Muriatic acid 31.45% (Klean‑Strip Green or any pool‑grade) | Dry acid (sodium bisulfate) adds sulfates that attack the cell over time |
...
| Item | Product |
|---|---|
| O‑rings, unions, lid | Silicone‑based pool lube (Magic Lube II, Jack's 327) |
| Never | Petroleum jelly, WD‑40, or plumber's grease on pool o‑rings |
| Threads | Teflon tape only on threaded plastic fittings; hand‑tight plus a quarter turn |

## Test kit

- Taylor K‑2006C (FAS‑DPD chlorine test, the one that actually reads high FC) plus the
  K‑1766 salt test, **or** TFTestkits TF‑100 with the salt kit.
- Keep reagents out of the sun; replace yearly.
...
5. **Second heat source** is not needed; the 110k heat pump is sized right for this
   volume in Virginia.

## Where the money should not go

- Mineral cartridges, ionizers, or "salt‑free" add‑ons. The iChlor already does the job.
- Enzyme and phosphate products on a schedule. Buy only when a test shows a problem.
- Weekly "shock" packets. A salt pool with correct CYA and FC does not need them.
